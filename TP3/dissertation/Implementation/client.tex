...
\begin{enumerate}
\item Client receives invite to chat from server on certain room id from user. [Store username in a queue]  
\item Client asks server the type of the room that it has been invited to. 
\item Server responds with 'Personal' or 'Group'
\item Client matches request with response from the queue
\item Client spawns the appropriate chat window, or updates the internal state information of an existing window with the new roomid. Information includes who is in the room, and the id to send messages to.
