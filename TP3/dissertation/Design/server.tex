...
\label{c2c}
No direct client-to-client communication is possible. Instead workers are able to indirectly communicate with other clients (i.e. clients to which they are not connected) by passing commands to the other client's worker. This is the server's main method of inter-client communication and relies heavily on the use of the buffers to pass messages from one worker to the network writer or command processor of another worker. This was do so that a client's worker was able monitor everything sent to and received from the client and update their own internal status in a reliable manner.
